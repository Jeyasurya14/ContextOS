...
def upgrade() -> None:
    ...
    if 'billing_events' not in existing_tables:
        ...
        idxs = get_indexes('billing_events')
        if 'ix_billing_events_event_type' not in idxs:
            op.create_index(op.f('ix_billing_events_event_type'), 'billing_events', ['event_type'], unique=False)
        if 'ix_billing_events_team_id' not in idxs:
            op.create_index(op.f('ix_billing_events_team_id'), 'billing_events', ['team_id'], unique=False)
        if 'ix_billing_events_user_id' not in idxs:
            op.create_index(op.f('ix_billing_events_user_id'), 'billing_events', ['user_id'], unique=False)
    ...
    if 'team_invitations' not in existing_tables:
        ...
        idxs = get_indexes('team_invitations')
        if 'ix_team_invitations_email' not in idxs:
            op.create_index(op.f('ix_team_invitations_email'), 'team_invitations', ['email'], unique=False)
        if 'ix_team_invitations_team_id' not in idxs:
            op.create_index(op.f('ix_team_invitations_team_id'), 'team_invitations', ['team_id'], unique=False)
        if 'ix_team_invitations_token' not in idxs:
            op.create_index(op.f('ix_team_invitations_token'), 'team_invitations', ['token'], unique=True)
    ...
    if 'teams' not in existing_tables:
        ...
        idxs = get_indexes('teams')
        if 'ix_teams_owner_id' not in idxs:
            op.create_index(op.f('ix_teams_owner_id'), 'teams', ['owner_id'], unique=False)
        if 'ix_teams_slug' not in idxs:
            op.create_index(op.f('ix_teams_slug'), 'teams', ['slug'], unique=True)
    ...
    if 'usage_records' not in existing_tables:
        ...
        idxs = get_indexes('usage_records')
        if 'ix_usage_records_date' not in idxs:
            op.create_index(op.f('ix_usage_records_date'), 'usage_records', ['date'], unique=False)
        if 'ix_usage_records_user_id' not in idxs:
            op.create_index(op.f('ix_usage_records_user_id'), 'usage_records', ['user_id'], unique=False)
