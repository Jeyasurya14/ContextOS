    ...
    <div className="animate-fade-in">
      ...
      {loading ? (
        ...
        <div className="card text-center py-12 animate-fade-in">
          ...
          <p className="text-dark-200 mb-1">No projects yet</p>
          <p className="text-sm text-dark-400">Create one to organize your context</p>
        </div>
      ) : (
        <div className="space-y-3">
          {projects.map((p) => (
            <div
              key={p.id}
              className="card group flex items-center justify-between hover:border-dark-600 transition-all duration-200 animate-slide-up"
              style={{ animationDelay: `${projects.indexOf(p) * 0.05}s` }}
            >
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="text-dark-400 hover:text-white transition-all duration-200 p-2 rounded-lg hover:bg-dark-800 group-hover:text-dark-300"
                >
                  Edit
                </button>
                <button
                  onClick={() => setDeletingProject(p)}
                  className="text-dark-400 hover:text-danger transition-all duration-200 p-2 rounded-lg hover:bg-dark-800 group-hover:text-dark-300"
                >
          ...
          <div className="relative card max-w-md w-full mx-4 p-6">
            ...
            <div className="space-y-3">
              <div>
                ...
                />
                <p className="text-xs text-dark-500 mt-1">{name.length} / 50</p>
              </div>
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-dark-800 border border-dark-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-brand/50 transition resize-none"
                />
              </div>
