...
// ============================================================================
// PRODUCTION TELEMETRY (privacy-respecting, opt-in)
// ============================================================================
...
class RateLimiter {
    ...
    update(maxRequests: number, windowMs: number): void {
        this.maxRequests = maxRequests;
        this.windowMs = windowMs;
    }
...
class CircuitBreaker {
    ...
    private setState(state: CircuitState): void {
        ...
        if (previous !== state) {
            ...
            console.log('[CircuitBreaker] State:', previous, '→', state);
        }
    }
...
class RequestCache {
    ...
    update(ttl: number): void { this.defaultTTL = ttl; }
    ...
    getStats(): { size: number } { return { size: this.cache.size }; }
}
...
// ============================================================================
// MAIN CHAT VIEW PROVIDER (Production Grade v2.1.0)
// ============================================================================
...
export class ChatViewProvider implements vscode.WebviewViewProvider {
    ...
    constructor(
        ...
        this._healthMonitor = new HealthMonitor(3);
        this._circuitBreaker = new CircuitBreaker(5, 30000, (state) => {
            if (state === CircuitState.OPEN) this._healthMonitor.recordFailure();
    ...
    // ── CONFIG RELOAD (called when settings change) ───────────────────────────
    public reloadConfig(): void {
        const config = vscode.workspace.getConfiguration('contextos');
        this._rateLimiter.update(
            config.get<number>('rateLimitMaxRequests') || 30,
            config.get<number>('rateLimitWindowMs') || 60000
        );
        this._requestCache.update(config.get<number>('cacheTTL') || 300000);
        this._telemetry.reload();
        console.log('[ChatViewProvider] Config reloaded');
    }
    ...
    private async _loadState(): Promise<void> {
        try {
            ...
            this._state = { ...this._state, messageHistory: history, conversationId };
    ...
    public resolveWebviewView(
        ...
        console.log('[ChatViewProvider] Webview initialized v2.1.0');
    }
    ...
    private _getHtmlForWebview(): string {
        ...
        const csp = [
            ...
            `connect-src ${apiOrigin === '*' ? '*' : apiOrigin + ' *'}`,
...
.live-dot{width:4px;height:4px;border-radius:50%;animation:pls 2.2s ease-in-out infinite}
.live-dot.green{background:var(--ok)}
.live-dot.red{background:var(--err)}
.live-dot.gray{background:var(--fg3)}
...
/* Messages */
#msgs{flex:1;overflow-y:auto;padding:8px 0 4px;display:flex;flex-direction:column;scroll-behavior:smooth}
...
/* Syntax tokens */
.kw{color:#c792ea}.str{color:#c3e88d}.cm{color:#546e7a;font-style:italic}.fn{color:#82aaff}.nm{color:#f78c6c}.tp{color:#ffcb6b}.op{color:#89ddff}.pk{color:#f97316}
...
.tok-count{margin-left:auto;font-family:var(--mono);font-size:9.5px;color:var(--fg3)}
...
#sndBtn.stop{background:linear-gradient(135deg,#f87171,#dc2626);box-shadow:0 2px 8px rgba(248,113,113,.3)}
.fmeta{display:flex;align-items:center;justify-content:space-between;padding:0 1px}
.hint{font-size:9.5px;color:var(--fg3)}
.model-chip{font-size:9.5px;color:var(--brand);font-weight:600;opacity:.65;display:flex;align-items:center;gap:3px}
.model-chip .dot{width:5px;height:5px;border-radius:50%}

/* Welcome */
.welcome{padding:16px 14px 10px;text-align:center;animation:mi .25s ease-out}
...
/* Onboarding banner (shown when no API key) */
.onboard{margin:10px 12px;padding:12px 13px;border-radius:var(--r);background:rgba(245,158,11,.06);border:1px solid rgba(245,158,11,.2);animation:mi .25s ease-out}
.onboard h3{font-size:12px;font-weight:700;color:var(--brand);margin-bottom:5px;display:flex;align-items:center;gap:6px}
.onboard p{font-size:11px;color:var(--fg2);line-height:1.5;margin-bottom:8px}
.onboard-steps{font-size:10.5px;color:var(--fg2);margin:0 0 10px;padding:0;list-style:none;display:flex;flex-direction:column;gap:3px}
.onboard-steps li{display:flex;align-items:center;gap:5px}
.onboard-steps li::before{content:attr(data-n);width:14px;height:14px;border-radius:50%;background:rgba(245,158,11,.15);border:1px solid rgba(245,158,11,.3);color:var(--brand);font-size:8px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;text-align:center;line-height:14px}
.ob-btn{padding:6px 14px;border-radius:7px;background:linear-gradient(135deg,#f59e0b,#d97706);color:#000;font-size:11px;font-weight:700;border:none;cursor:pointer;font-family:var(--font);transition:opacity .14s}
...
<div id="hdr">
  <div class="hdr-l">
    ...
    <span class="live-badge" id="statusBadge"><span class="live-dot gray" id="liveDot"></span><span id="badgeLabel">Loading</span></span>
  </div>
  <div class="hdr-r">
...
<div id="ftr">
  <div class="irow">
    ...
    <button id="sndBtn" title="Send (Enter)">
      <svg id="sendIcon" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        <line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon>
      </svg>
      <svg id="stopIcon" width="12" height="12" viewBox="0 0 24 24" fill="currentColor" style="display:none">
  ...
  <div class="fmeta">
    ...
    <span class="model-chip" id="modelChip">
      <span class="dot" id="modelDot" style="background:var(--fg3)"></span>
...
(function() {
  ...
  function setStatus(state) {
    // state: 'loading' | 'ready' | 'offline' | 'nokey'
    liveDot.className = 'live-dot';
    if (state === 'ready') {
      liveDot.classList.add('green');
      badgeLabel.textContent = 'Live';
      modelDot.style.background = 'var(--ok)';
      modelLabel.textContent = 'GPT-4o · Connected';
    } else if (state === 'offline') {
      liveDot.classList.add('red');
      badgeLabel.textContent = 'Offline';
      modelDot.style.background = 'var(--err)';
      modelLabel.textContent = 'API unreachable';
    } else if (state === 'nokey') {
      liveDot.classList.add('red');
      badgeLabel.textContent = 'No key';
      modelDot.style.background = 'var(--err)';
      modelLabel.textContent = 'API key required';
      showOnboardingBanner();
    } else {
      liveDot.classList.add('gray');
      badgeLabel.textContent = 'Loading';
      modelDot.style.background = 'var(--fg3)';
      modelLabel.textContent = '—';
    }
  }

  // ── Onboarding banner (no key) ────────────────────────────────────────────

  function showOnboardingBanner() {
    if (document.getElementById('onboardBanner')) return;
    const banner = document.createElement('div');
    banner.id = 'onboardBanner';
    banner.className = 'onboard';
    banner.innerHTML = \`
      <h3>&#128273; API Key Required</h3>
      <p>Connect ContextOS to your account to start chatting. It only takes 30 seconds.</p>
      <ol class="onboard-steps">
        <li data-n="1">Open the <strong>ContextOS dashboard</strong> and go to Settings &rarr; API Keys</li>
        <li data-n="2">Click <strong>Generate API Key</strong> and copy the key</li>
        <li data-n="3">Click the button below and paste your key</li>
      </ol>
      <button class="ob-btn" id="obBtn">Set API Key</button>
    \`;
    msgsEl.insertBefore(banner, msgsEl.firstChild);
    document.getElementById('obBtn').onclick = () => {
      vscode.postMessage({ type: 'runCommand', command: 'contextos.setApiKey' });
    };
  }
  ...
  const lock = v => {
    ...
    if (v) {
      ...
      sndBtn.classList.add('stop');
      sndBtn.disabled = false; // keep enabled for stop
    } else {
      sendIcon.style.display = '';
      stopIcon.style.display = 'none';
      sndBtn.classList.remove('stop');
  ...
  const showStatus = label => {
    ...
    sbarIn.innerHTML = \`<div class="spin"></div><span style="font-style:italic">\${esc(label)}</span><span class="tok-count" id="tokC"></span>\`;
  };

  const hideStatus = () => { sbar.classList.remove('on'); sbarIn.innerHTML = ''; };

  const clearWelcome = () => { const w = document.getElementById('welcome'); if (w) w.remove(); };

  // ── Syntax highlighter (minimal, no external deps) ────────────────────────
  ...
  function highlight(code, lang) {
    ...
    if (['js','javascript','ts','typescript','jsx','tsx'].includes(l)) {
      s = s
        ...
        .replace(/\\b(const|let|var|function|class|return|import|export|from|async|await|if|else|for|while|new|typeof|instanceof|of|in|try|catch|finally|throw|default|switch|case|break|continue|void|delete|yield|extends|super|static|this)\\b/g,'<span class="kw">$1</span>')
        .replace(/\\b([A-Z][a-zA-Z0-9_]*)(?=[(])/g,'<span class="fn">$1</span>')
        .replace(/\\b([A-Z][a-zA-Z0-9_]*)\\b/g,    '<span class="tp">$1</span>')
        .replace(/\\b(\\d+\\.?\\d*)\\b/g,           '<span class="nm">$1</span>');
    } else if (['py','python'].includes(l)) {
      s = s
        .replace(/#.+/g,                 '<span class="cm">$&</span>')
        .replace(/(".*?"|\'.*?\'|"""[\\s\\S]*?"""|\'\'\'[\\s\\S]*?\'\'\')/g,'<span class="str">$1</span>')
        .replace(/\\b(def|class|import|from|return|if|elif|else|for|while|in|not|and|or|is|lambda|with|as|try|except|finally|raise|pass|break|continue|yield|global|nonlocal|async|await|True|False|None)\\b/g,'<span class="kw">$1</span>')
        .replace(/\\b([a-z_][a-z_0-9]*)(?=[(])/g, '<span class="fn">$1</span>')
        .replace(/\\b(\\d+\\.?\\d*)\\b/g,           '<span class="nm">$1</span>');
    } else if (['bash','sh','shell','zsh'].includes(l)) {
      s = s
        .replace(/#.+/g,              '<span class="cm">$&</span>')
        .replace(/(".*?"|'.*?')/g,    '<span class="str">$1</span>')
        .replace(/\\b(if|then|else|fi|for|do|done|while|case|esac|function|return|export|echo|cd|ls|mkdir|rm|cp|mv|cat|grep|awk|sed|curl|wget|npm|npx|python|pip|git|docker)\\b/g,'<span class="kw">$1</span>')
        .replace(/\\$(\\w+)/g,          '<span class="pk">$$$1</span>');
    }
    return s;
  }

  // ── Markdown renderer (fixed, proper template literal) ────────────────────
  ...
  function renderMd(raw) {
    ...
    text = text.replace(/((<li>(?!\\s*<li class="ol">).*<\\/li>\\n?)+)/g, m => '<ul>' + m + '</ul>');
    ...
    text = text.replace(/CBPH_(\\d+)_END/g, (_, i) => {
      ...
      if (!b) return '';
      const highlighted = highlight(b.code, b.lang);
      return '<div class="cb"><div class="cb-hdr"><span class="cb-lang">' + esc(b.lang) +
        '</span><button class="cb-copy" data-id="' + b.id + '" onclick="window._copyCode(this)">\u2398 Copy</button></div>' +
  ...
  window._copyCode = btn => {
    ...
    vscode.postMessage({ type: 'copyCode', id, value: codeMap[id] || '' });
  };
  ...
  const addUser = text => {
    ...
    g.className = 'mg user';
    const av = document.createElement('div'); av.className = 'av av-u'; av.textContent = 'U';
    const mc = document.createElement('div'); mc.className = 'mc'; mc.textContent = text;
    const cp = document.createElement('button'); cp.className = 'msg-copy'; cp.textContent = 'Copy';
    cp.onclick = () => { vscode.postMessage({ type: 'copyMsg', value: text }); cp.textContent = 'Copied!'; setTimeout(() => cp.textContent = 'Copy', 1800); };
    const ts = document.createElement('div'); ts.className = 'ts'; ts.textContent = now();
    const right = document.createElement('div');
    right.style.cssText = 'flex:1;min-width:0;max-width:calc(100% - 33px)';
  ...
  const startBot = () => {
    ...
    const av = document.createElement('div'); av.className = 'av av-b';
    ...
    const raw = document.createElement('span'); raw.className = 'raw';
    const cur = document.createElement('span'); cur.className = 'cur'; cur.innerHTML = '\u25ae';
    mc.appendChild(raw); mc.appendChild(cur);
    g.appendChild(av); g.appendChild(mc);
    msgsEl.appendChild(g); scrollBot(true);
    return { group: g, mc, rawEl: raw, cur };
  };
  ...
  const finishBot = () => {
    ...
    const cp = document.createElement('button'); cp.className = 'msg-copy'; cp.textContent = 'Copy';
    cp.onclick = () => { vscode.postMessage({ type: 'copyMsg', value: raw }); cp.textContent = 'Copied!'; setTimeout(() => cp.textContent = 'Copy', 1800); };
    bot.group.appendChild(cp);
    const ts = document.createElement('div'); ts.className = 'ts'; ts.style.marginTop = '4px'; ts.textContent = now();
    bot.mc.appendChild(ts);
    bot = null;
  };

  const addErr = (text, canRetry, originalMessage) => {
    const g = document.createElement('div'); g.className = 'mg err';
    const av = document.createElement('div'); av.className = 'av av-b';
    const mc = document.createElement('div'); mc.className = 'mc';
    ...
    if (canRetry && originalMessage) {
      ...
      rb.onclick = () => { vscode.postMessage({ type: 'retry', message: originalMessage }); g.remove(); };
      mc.appendChild(rb);
    }
  ...
  sndBtn.addEventListener('click', e => { e.preventDefault(); send(); });
  ...
  window.addEventListener('message', event => {
    ...
    switch (m.type) {
      ...
      case 'searching':
        showStatus(m.source ? 'Searching ' + m.source + '\u2026' : 'Searching\u2026');
        if (!bot) bot = startBot();
        break;
      ...
      case 'token':
        ...
        const tokEl = document.getElementById('tokC');
        if (tokEl) tokEl.textContent = '\u2248' + Math.round(tokenCount / 4) + ' tokens';
        scrollBot(false);
        break;
      ...
      case 'done':
        hideStatus(); finishBot(); lock(false); inp.focus();
        break;
      ...
      case 'addContext': {
        const ban = document.createElement('div'); ban.className = 'ctxban';
      ...
      case 'keyStatus':
        hasKey = m.hasKey;
        setStatus(m.hasKey ? 'ready' : 'nokey');
        break;
  ...
  inp.focus();
  setStatus('loading');
  console.log('[Webview] ContextOS Chat UI v2.1.0 initialized');
})();
</script>
</body>
</html>`;
                ...
                switch (data.type) {
                    ...
                    case 'abort':
                        if (this._activeController) {
                            ...
                            console.log('[ChatViewProvider] Request aborted by user');
                        }
                        break;
                    ...
                    case 'retry':
                        if (data.message) await this._handlePrompt(data.message, webviewView);
                        break;

                    case 'ready': {
                        // Check key and send status
                        const apiKey = await this._context.secrets.get('contextos_api_key');
                        webviewView.webview.postMessage({ type: 'keyStatus', hasKey: !!apiKey });

                        // Restore history
                        if (this._state.messageHistory.length > 0) {
                            webviewView.webview.postMessage({
                                type: 'restoreHistory',
                                messages: this._state.messageHistory,
                                scrollPosition: this._lastScrollPosition
    ...
    private async _handlePrompt(question: string, webviewView: vscode.WebviewView): Promise<void> {
        ...
        if (!this._rateLimiter.tryAcquire()) {
            const waitMs = this._rateLimiter.getResetTime() - Date.now();
            webviewView.webview.postMessage({
                type: 'error',
                value: `Rate limit exceeded. Please wait ${Math.ceil(waitMs / 1000)}s before sending another message.`,
                canRetry: true,
        ...
        if (this._circuitBreaker.getState() === CircuitState.OPEN) {
            webviewView.webview.postMessage({
                type: 'error',
                value: 'Service temporarily unavailable. Please wait a moment and try again.',
                canRetry: true,
                originalMessage: question
            });
            return;
        ...
        if (!apiKey) {
            webviewView.webview.postMessage({
                type: 'error',
                value: 'No API key configured.\n\nOpen the Command Palette and run "ContextOS: Set API Key".',
                canRetry: false
            });
            return;
        }
        ...
        this._state.messageHistory.push({ role: 'user', content: question, timestamp: Date.now() });
        await this._saveState();
        ...
        try {
            ...
            this._state.messageHistory.push({ role: 'assistant', content: response, timestamp: Date.now() });
            ...
            if (!msg.includes('AbortError') && !msg.toLowerCase().includes('aborted')) {
                ...
                webviewView.webview.postMessage({
                    type: 'error',
                    value: msg || 'Failed to get response. Please try again.',
                    canRetry: true,
                    originalMessage: question
                });
    ...
    private async _executeRequestWithRetry(
        params: { question: string; apiKey: string; apiUrl: string; maxRetries: number; timeoutMs: number },
        webviewView: vscode.WebviewView
    ): Promise<string> {
        ...
        for (let attempt = 0; attempt <= params.maxRetries; attempt++) {
            try {
                if (attempt > 0) {
                    ...
                    console.log(`[ChatViewProvider] Retry ${attempt} after ${backoffMs}ms`);
                    await new Promise(resolve => setTimeout(resolve, backoffMs));
                }

                return await this._circuitBreaker.execute(() => this._makeRequest(params, webviewView));

            } catch (error: any) {
                lastError = error;

                // Don't retry on abort or auth errors
                if (this._shouldNotRetry(error)) break;
                console.error(`[ChatViewProvider] Attempt ${attempt + 1} failed:`, (error as Error).message);
            }
        }
    ...
    private async _makeRequest(
        params: { question: string; apiKey: string; apiUrl: string; timeoutMs: number },
        webviewView: vscode.WebviewView
    ): Promise<string> {
        ...
        if (cached) {
            ...
            webviewView.webview.postMessage({ type: 'token', content: cached });
        ...
        const timeoutId = setTimeout(() => this._activeController?.abort(), params.timeoutMs);
        ...
        console.log('[ChatViewProvider] → POST', `${params.apiUrl}/api/v1/query`);
        ...
        try {
            ...
            if (result) this._requestCache.set(cacheKey, result);
    ...
    private async _handleStreamEvent(data: any, webviewView: vscode.WebviewView): Promise<void> {
        switch (data.event) {
            case 'thinking':
                webviewView.webview.postMessage({ type: 'thinking', message: data.message || 'Thinking…' });
                break;
            case 'searching':
                webviewView.webview.postMessage({ type: 'searching', source: data.source, count: data.count });
                break;
            case 'token':
                webviewView.webview.postMessage({ type: 'token', content: data.content });
                break;
            case 'sources':
                webviewView.webview.postMessage({ type: 'sources', sources: data.sources });
                break;
            case 'done':
                if (data.conversation_id) this._state.conversationId = data.conversation_id;
                webviewView.webview.postMessage({ type: 'done' });
                break;
            case 'error':
    ...
    public notifyKeySet(): void {
        if (this._view) {
            this._view.webview.postMessage({ type: 'keySet' });
        }
    }
