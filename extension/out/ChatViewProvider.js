...
class ChatViewProvider {
    ...
    async _loadConversationHistory() {
        try {
            const history = this._context.globalState.get('messageHistory', []);
            const conversationId = this._context.globalState.get('conversationId');
    ...
    resolveWebviewView(webviewView, _ctx, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(async (data) => {
            ...
            try {
                ...
                webviewView.webview.postMessage({
                    type: 'error',
                    value: 'Internal error: ' + error.message
                });
            }
        });
    }
    ...
    async _handlePrompt(question, webviewView) {
        ...
        const apiUrl = config.get('apiUrl') || 'https://contextos-api-jxdr.onrender.com';
